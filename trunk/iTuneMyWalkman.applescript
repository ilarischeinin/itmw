...
on movepics(myprocess)
	...
	repeat with x in myexclude
		if x as string ≠ "" then set excludestring to excludestring & " -not -iname " & (quoted form of x) & " -not -ipath " & quoted form of ("*/" & x & "/*")
	end repeat
...
on getsongs()
	...
	if mysizelimit < 0 then
		...
		repeat with x in myexclude
			if x as string ≠ "" then set excludestring to excludestring & "-I " & (quoted form of x) & " "
		end repeat
